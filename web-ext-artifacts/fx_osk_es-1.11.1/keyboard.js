...
FxKeyNumpad = JSON.stringify({
    ...
    "main": [
        [
            ...
            ["0"], [{ "label": "Borrar", "special": 8, "flex": 1, "type": "repeat" }]
        ], [
    ...
    _toggleOpen: function (open) {
        ...
        if (open) {
            if (!this.hierarchy.isMaster) {
                ...
                kb.style.width = '100%',//this.getMaxWidth(this.activeOSK) * this.settings.scale + "px";
                    kb.style.height = (this.getMaxHeight(this.activeOSK) * this.settings.scale) + "px"; // 34%
                kb.style.padding = this.settings.padding * this.settings.scale + "px";
                kb.style.fontFamily = "arial,sans-serif";
                kb.style.color = "#000000";
                kb.style.fontSize = 28 + this.settings.scale + "px";
                kb.style.borderRadius = 5 * this.settings.scale + "px";


                //horizontal 
                if (window.innerWidth > window.innerHeight) {
                    this.settings.key_height = window.innerWidth * 0.035
                    kb.style.fontSize = 28 * this.settings.scale + "px";
                    kb.style.height = (this.getMaxHeight(this.activeOSK) * this.settings.scale) - 135 + "px"; // 34%
                }

                /*console.log("kb.style.height", kb.style.height);
                console.log("xxx", window.innerWidth, window.innerHeight,this.getMaxWidth(this.activeOSK), this.getMaxHeight(this.activeOSK), );*/

                for (const element of kb.childNodes) {
                    for (const chNodes of element.childNodes) {
                        chNodes.style.width = this.settings.key_height * this.settings.scale + 15 + "px";
                        ...
                        //horizontal 
                        if (window.innerWidth > window.innerHeight) {
                            chNodes.style.width = this.settings.key_height * this.settings.scale + 50 + "px";
                        }

                    }
                }

                console.log("kb", kb.style.width);
